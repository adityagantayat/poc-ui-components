...
const Toast = ({ message, onClose, id, animation, variant='default', mode, icon }: ToastProps) => {
...
const modeColor = mode === 'dark' ? 'bg-gradient-to-t from-gray-800 via-slate-800 to-gray-800 text-white' 
                                    : 'bg-white text-black';
const getVariantStyles = () => {
  ...
  const { border, boxShadow } = getVariantStyles();
  ...
  return (
    <motion.div
      className={`toast p-1 rounded-lg shadow-lg mb-3 flex justify-between items-center ${modeColor} ${border}`}
